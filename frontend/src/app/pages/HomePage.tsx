...
export function HomePage() {
  ...
  const filteredCafes = mockCafes.filter((cafe) => {
    ...
    const matchesLocation = selectedLocation === "전체" || cafe.location.includes(selectedLocation);
    ...
    <div className="min-h-screen bg-gray-50">
      <section className="bg-gradient-to-r from-pink-500 to-purple-600 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl mb-8 text-pink-100">내 주변의 메이드 카페를 쉽게 찾고 예약하세요</p>
            ...
            <Link key={cafe.id} to={`/cafe/${cafe.id}`}>
              <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer h-full">
                <div className="relative">
                  ...
                  {cafe.tags.map((tag) => (
                    <Badge key={tag} className="absolute top-2 left-2 bg-pink-600">
                      {tag}
                    </Badge>
                  ))}
                </div>
